...
namespace MHSJ.Web.admin.usercontrols
{
    public partial class upfilemanager : UserControl
    {
        /// <summary>
        ///     允许上传的文件格式
        /// </summary>
        ...
        protected void Delete()
        {
            ...
            if (PageUtils.CurrentUser.Type != (int)UserType.SuperAdministrator)
            {
                Response.Redirect(FileName + "?path=" + return_deletepath + "&result=555");
            }
        ...
        {
            var directoryName = DirectoryName.Text;
            if (string.IsNullOrWhiteSpace(directoryName.Trim()))
            {
                ShowError("文件夹名不能为空！");
                return;
            }

            var dircUrl = path + "/" + directoryName;
            string saveDirectory = Server.MapPath(dircUrl); //文件保存绝对文件夹
        ...
        protected void btnUpload_Click(object sender, EventArgs e)
        {
            ...
            string uploadPath = GetPath ; //文件保存相对路径
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    //    string newFileName = Path.GetFileNameWithoutExtension(postedFile.FileName) + "w(" + iCounter +
