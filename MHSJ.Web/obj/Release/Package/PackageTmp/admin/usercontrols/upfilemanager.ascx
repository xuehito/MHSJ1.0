<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="upfilemanager.ascx.cs" Inherits="MHSJ.Web.admin.usercontrols.upfilemanager" %>
<%@ Import Namespace="System.IO" %>
<%@ Import Namespace="MHSJ.Core.Common" %>
...
<h2>附件管理</h2>
 ...
 <div  class="right">           
   ...
   <%-- <p><%=ConfigHelper.SitePath  %>upfiles/<%=DateTime.Now.ToString("yyyyMM") %></p>--%>
    ...
    <%--<p><asp:CheckBox ID="chkWatermark" runat="server" Text="加水印(图片)" Checked="False" /> <a href="setting.aspx#watermarks" class="small">水印详细设置</a> </p>--%>
    <p>存在同名文件时:</p>
    <p> 
    ...
    <br>
    <h4>新建文件夹</h4>
    <p><asp:TextBox runat="server" ID="DirectoryName"></asp:TextBox></p>
    <p><asp:Button ID="btnAdd" runat="server"  Text="新建" onclick="btnAdd_Click"  CssClass="button"/></p>
</div>

<div class="left" >
    <h4>当前位置: <%=GetPathUrl()%></h4>
    <ul class="upfile" id="box">
    <%
        foreach (FileSystemInfo d in currentDirectory.GetFileSystemInfos().OrderByDescending(info => info.CreationTime))
            {
    %>
        <li>
          ...
          { %>
                <a href="<%=path+d.Name %>" target="_blank" onclick=" return returnValue('<%=path+d.Name %>','<%=d.Extension %>');">
                 ...
                 <%if (IsImage(d.Extension))
                   { %>
                    <img class="lazy" data-original="<%=path+d.Name %>" width="48" height="48"/>
                 <%}
                   else
                   { %>
                    <img class="lazy" data-original="../common/images/file/<%=GetFileImage(d.Extension) %>"  width="48" height="48"/>
                 <%} %>
